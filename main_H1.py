...
def findbasescaling(noise, TA_tf):
    ...
    while step < 4:
        ...
        if pcethres < 50 and step == 0:
            ...
            step += 1
    if fail is True:
        bestscaling = 0.63 #estimated empirically
    return bestscaling
        ...
        for video_path in videos_path:
            ...
            if not os.path.exists(out_file):
                ...
                for idx_frame in set_idx_frame:
                    ...
                    out_name1 = FLAGS.output + '/' + video_path[len(test_set):-4] + '_PCE.mat'
                    savemat(out_name1, mdir)
                    mdir = {'time': np.asarray(time_array)}
                    out_name2 = FLAGS.output + '/' + video_path[len(test_set):-4] + '_time.mat'
